    ...
    if Config.FORCE_SUB:
        try:
            ...
            button = [[InlineKeyboardButton(
                'join Updates channel', url=f'https://t.me/TMWAD')]]
            ...
            logger.warning(f"Make me admin in @TMWAD")
            if m.from_user.id in Config.AUTH_USERS:
                return await m.reply_text("Make me admin in @TMWAD")

        except UsernameNotOccupied:
            logger.warning(
                "The forcesub username was Incorrect. Please give the correct username.")
            if m.from_user.id in Config.AUTH_USERS:
            ...
            if "belongs to a user" in str(e):
                logger.warning(
                    "Forcesub username must be a channel username Not yours or any other users username")
                if m.from_user.id in Config.AUTH_USERS:
                    return await m.reply_text("Forcesub username must be a channel username Not yours or any other users username")
            logger.error(e)
            return await m.reply_text("Some thing went wrong. Ask [Here](https://t.me/TMWAD/17", disable_web_page_preview=True, quote=True)
